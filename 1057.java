...
class Solution {
    public int[] assignBikes(int[][] workers, int[][] bikes) {
        ...
        int[] workerVisited = new int[m];
        int[] bikeVisited = new int[n];
        int count = 0;
        for (int dis = 0; dis < 2000; dis++) {
            for (int[] pair : distance[dis]) {
                // System.out.println(pair[0] + "," + pair[1] + "," + dis);
                if (workerVisited[pair[0]] == 0 && bikeVisited[pair[1]] == 0) {
                    workerVisited[pair[0]] = 1;
                    bikeVisited[pair[1]] = 1;
                    assign[pair[0]] = pair[1];
                    count++;
