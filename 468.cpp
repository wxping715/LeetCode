...
public:
    string validIPAddress(string IP) {
        int idx = IP.find('.');
        if (idx == -1 && checkIpv6(IP)) return "IPv6";
        else if(checkIpv4(IP)) return "IPv4";
        return "Neither";
    }
    ...
    bool checkIpv4(string ip) {
        ...
        for (int i = 0; i < ip.length(); i++) {
            ...
            if (i+1 == ip.length() && ip[i] == '.') return false;
            
            part++;
            if (cur.length() == 0 || cur.length() > 3 ||
                (cur.length() > 1 && cur[0] == '0') ||
                (stoi(cur) > 255 || stoi(cur) < 0)) 
                    return false;
        }
        // cout << part << endl;
        return part == 4;
    ...
    bool checkIpv6(string ip) {
        ...
        for (int i = 0; i < ip.length(); i++) {
            cur = "";
            while (i < ip.length() && ip[i] != ':') {
                if (!isIpv6Char(ip[i])) return false;
                cur.push_back(ip[i]);
                i++;
            }
            if (i+1 == ip.length() && ip[i] == ':') return false;
